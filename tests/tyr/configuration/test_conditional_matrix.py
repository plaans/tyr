...
class TestConditionalMatrixExpansion:
    ...
    def test_simple_string_conditional(self):
        ...
        configs = [
            {
                "name": "planner-{type}",
                "matrix": {"type": ["flexible", "strict"]},
                "env": {"SYMMETRY": "{type == 'flexible' ? 'psp' : 'simple'}"},
            }
        ]
        ...
        configs = [
            {
                ...
                "env": {"MODE": "{strategy.timeout > 100 ? 'extended' : 'quick'}"},
            }
        ]
        ...
        configs = [
            {
                ...
                "matrix": {"type": ["fast", "slow"], "mode": ["debug", "release"]},
                "env": {
                    "TYPE": "{type}",
                    "OPTIMIZATION": "{mode == 'release' ? 'O3' : 'O0'}",
                    "DESCRIPTION": (
                        "Running {type} with {mode == 'debug' ? 'debugging' : 'optimizations'}"
                    ),
                },
            }
        ]
        ...
        configs = [
            {
                ...
                "env": {"STATUS": "{enabled == true ? 'active' : 'inactive'}"},
            }
        ]
    ...
    def test_invalid_conditional_syntax(self):
        ...
        assert (
            config["env"]["INVALID_2"] == "{var == 'value'}"
        )  # Invalid conditional, left unchanged
        ...
        configs = [
            {
                "name": "test",
                "matrix": {"var": ["value"]},
                "env": {"RESULT": "{missing_var == 'value' ? 'yes' : 'no'}"},
            }
        ]
